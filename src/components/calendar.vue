        ...
        <table cellpadding="5">
            ...
            <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
                <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}"
                    ...
                    <span class="text" :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}"
                         v-if="child.eventName!=undefined">￥{{child.eventName}}</span>
                    <div class="text"
    ...
    export default {
        ...
        methods: {
            ...
            select(k1, k2,e) {
                ...
                // 日期范围
                var mewsyear=this.year;
                var mewsmonth=this.month + 1;
                var mewday=this.days[k1][k2].day;

                //let currentTime=[mewsyear+ "," + mewsmonth+ "," + mewday]
                //let newprice = this.events[mewsyear+ "," + mewsmonth+ "," + mewday]
                 //if( newprice){
                     //this.price.push(newprice);
                     //this.$root.$emit("calendarPriceSet",this.price);
                     //this.$root.$emit("calendarTime",currentTime)
                 //}
