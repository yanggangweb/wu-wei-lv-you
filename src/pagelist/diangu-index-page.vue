<template type="text/html" lang="jade">
    f7-page(:class="[name,'diangu-index-page']")
        f7-navbar(sliding="")
            f7-nav-left
        ...
        f7-block.banner
            h2.title-main.nowrap-ellipsis {{_get(data,'name')}}
        f7-block.body
            f7-list.mlist()
                f7-list-item(v-if="data.merchantId",@click="routeTo(`/jingdian-index-page/${_get(data,'merchantId')}/JINGDIAN/`)")
                    h3.text-inds
                        f7-link(icon="i-dingwei2",color="red",)
                            b(v-html="_get(data,'linkName')")
                f7-list-item
            ...
            .intro-tx-title
                ...
                .comment-list.white-bg.no-h-gap
                    h4.title
                        .left
                            b 评论
                            star-list(:value="_get(avgScore)",:no-edit="true")
                            span.number {{totalNum||0}}条评论
                        f7-link
