<template type="text/html" lang="jade">
    f7-page(:class="[name]")
        ...
        f7-block.body(v-if="_size(data)")
            f7-button.custom-dish(fill,@click="routeTo(`/techan-dishlist/${id}/${mark}/-1`)") 自选
            f7-list.mlist
                ...
                f7-list-item
                    .mapCont
                        .button-cont
                            f7-button.ib.color-flat-green(fill,@click="routeTo(`/map-around/周边-${mer.data.name}/${mer.data.longitude},${mer.data.latitude}`)") 周边
                            | &nbsp;
                            f7-button.ib.color-flat-green(fill,@click="directTo(data)")  导航
                        el-amap.map(:zoom="13",:center="[data.longitude,data.latitude]")
                ...
                f7-list-item
                    .text-introduce
                        ...
                        //(v-html="got('mer.merchantInfo.introduction')" )
                ...
                template(v-for="pd in pdList")
                    ...
                    f7-list-item.product-desc
                        .dish-wrapper
                            h3 菜品内容
                            .dish-list(@click="routeTo(`/techan-dishlist/${id}/${mark}/${pd.id}`)")
                                i.icon.i-lp_speaker
        ...
        methods:{
            ...
            buy:function(pd){
                ...
                var para = {
                    ...
                    merchantId      :   m.id,
                    comboId         :   pd.id,
                    //totalPrice      :   _.sum(pd.list.map(el=>el.price*el.quantity)), //单个商品累加价格
                ...
                m.routeTo(`/techan-order/${m.id}/${m.mark}/${para}`,1);
            },
